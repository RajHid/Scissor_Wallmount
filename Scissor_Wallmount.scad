    ...
    see_me_in_colourful(){
        translate([0,0,0]){
            difference(){
                Scissor_Wall_Mount();
                hull(){
                    translate([0,(-(InnerSpace_Y+WallThikness*2))/2,Object_Heigth/3+(InnerSpace_Y+WallThikness)]){
                        rotate([0,90,0]){
                            cylinder(h=2*(InnerSpace_X+WallThikness*2),d=2*(InnerSpace_Y+WallThikness),center=true);
                        }
                    }
                    translate([0,0,Object_Heigth/3+(InnerSpace_Y+WallThikness)]){
                        rotate([0,90,0]){
                            cylinder(h=2*(InnerSpace_X+WallThikness*2),d=2*(InnerSpace_Y+WallThikness),center=true);
                        }
                    }
                }
                translate([0,ScissorHandlThickness/2+WallDistance,Object_Heigth/3]){
                    Press_Fit_Cut(-90,Object_Heigth,InnerSpace_Y+WallDistance,ScissorHandlThickness);
                }
                translate([0,(InnerSpace_Y+WallThikness)/2-WallThikness/4+WallDistance,3*Object_Heigth/4]){
                    rotate([-90,0,0]){
                        Screwcutter(100,5,100,3,0.3,3.5);
                    }
                }
            }
        }
        ...
        translate([0,0,0]){
            if(CUT_MODULES_RENDERED=="true"){
                ...
                translate([0,ScissorHandlThickness/2,Object_Heigth/3]){
                    //Press_Fit_Cut(-90,Object_Heigth,InnerSpace_Y,ScissorHandlThickness);
                }
                translate([0,(InnerSpace_Y+WallThikness)/2-1.5,3*Object_Heigth/4]){
                    rotate([-90,0,0]){
                        //Screwcutter(100,5,100,3,0.3,3.5);
                    }
                }
...
// =--------------------------------- Modules -----------------------------------=
// ===============================================================================
module Scissor_Wall_Mount(){
    LinExt_Base_Schape_Ring();
}
...
// ===============================================================================
//LinExt_Base_Schape_Ring();
module LinExt_Base_Schape_Ring(){
linear_extrude(Object_Heigth){
    Base_Schape_Ring();
    }    
}
...
module Base_Schape_Ring(){
    ...
    difference(){
        union(){
            2D_Rounded_Square_Base_Shape(InnerSpace_X+WallThikness*2,InnerSpace_Y+WallThikness*2,0.2);
            translate([0,(InnerSpace_Y+WallThikness*2+WallDistance)/2,0]){
                square([InnerSpace_X+WallThikness*2,WallDistance],center=true);
            }
        }
        Smooth(r=2){
            Fillet(r=1){
                square([InnerSpace_X,InnerSpace_Y],center=true);
                square([InnerSpace_X+2*ScissorBolt_H,ScissorBolt_D],center=true);
            }
        }
        ScaleCircle_y=(InnerSpace_Y+WallThikness*2)/(InnerSpace_X+WallThikness*2);
        difference(){
            square([2*(InnerSpace_X+WallThikness*2),InnerSpace_Y*2],center=true);
            scale([1,ScaleCircle_y]){
                circle(d=InnerSpace_X+WallThikness*2);
            }
            translate([0,InnerSpace_Y,0]){
                square([2*(InnerSpace_X+WallThikness*2),InnerSpace_Y*2],center=true);
            }
        }
    }
...
//Smooth(r=3){square([40,10]);square([10,40]);}
module Smooth(r=3){
    //$fn=30;
    offset(r=r,$fn=30){
        offset(r=-r,$fn=30){
...
//Fillet(r=3){square([40,10],center=true);square([10,40],center=true);}
module Fillet(r=3){
    //$fn=30;
    offset(r=-r,$fn=30){
        offset(r=r,$fn=30){
...
// Inner and outer Fillet =====================================
// Smooth(r=3){Fillet(r=3){square([40,10]);square([10,40]);}}
// ================================================================
